...
- README landing-page proof was added:
  ...
  - `docs/assets/skratched-workspace.png`, `docs/assets/skratched-workspace-scout.png`, `docs/assets/skratched-context-map.png`, and `docs/assets/skratched-redacted-export.png` are tracked screenshot assets.
...
- Workspace Scout was implemented:
  - `POST /api/workspace/scan-preview` scans user-approved local roots and records metadata-only `workspace_scans` / `workspace_candidates` rows for matching secrets, configs, code, screenshots, docs, or exact filenames.
  - Scout previews include path, relative path, file type, media type, size, mtime, stat fingerprint, duplicate/capture status, scan counts, and index age without reading file bodies.
  - `POST /api/workspace/capture` turns a selected candidate into a normal Skratched item, defaulting to metadata-only capture and requiring explicit local unlock before importing secret-like file content.
  - The browser UI includes a compact Workspace Scout panel with root/type/since/depth controls and candidate cards with `Capture Metadata` and `Import Content` actions.
  - README and screenshot generation now include a real Workspace Scout UI/API screenshot generated from a temporary local demo workspace.
